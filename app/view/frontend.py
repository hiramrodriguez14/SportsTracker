...
def signup(email, username, password):
    try:
        ...
        result, status = user_handler.signup(signup_data)
...
def show_dashboard():
    ...
    if prompt := st.chat_input("Ask me anything about exercises!"):
        ...
        with st.chat_message("assistant"):
            ...
            full_response = ""
            assistant_response, st.session_state.history, context = bot.chat(prompt, st.session_state.history)
    ...
    if selected_query == "Exercises by Muscle Group":
        muscle_options = [
            "abductors", "biceps", "lower back", "glutes", "calves", "quadriceps",
            "shoulders", "triceps", "neck", "adductors", "chest", "abdominals",
            "hamstrings", "forearms", "traps", "middle back", "lats"
        ]
    ...
    if confirm_button:
        if selected_query == "Top 3 Teams with Most Championships":
            ...
            cleaned_muscle = muscle_text.strip().lower()
            try:
                ...
                if data:
                    ...
                    st.warning(f"No exercises found for: '{muscle_text}'")
            except Exception as e:
